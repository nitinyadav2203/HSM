...
void addDoc()
{
    ...
    fileObj<<d.d_ID<<endl<<d.d_name<<endl<<d.d_age<<endl<<d.qual<<endl<<spec<<endl<<d.experience<<endl<<d.d_city<<endl;
    fileObj.close();
...
void editDoc(doctor &docobj)
{
    ...
    int age,exp,count=0;
    string ID,name,ci,qua,spe;
    while(fileObj>>ID>>name>>age>>qua>>spe>>exp>>ci)
    {
        if(ID==x)
        {
            cout<<"\n\n1.ID:"<<ID<<endl<<"2.Name:"<<name<<endl<<"3.Age:"<<age<<endl<<"4.Qualification:"<<qua<<endl<<"5.Specialization:"<<spe<<endl<<"6.Experience:"<<exp<<endl<<"7.City:"<<ci<<endl<<endl;
            count++;
            break;
        }
    }
...
//    	cout << "Error opening file";
    ...
    while(file>>line>>name1>>age1>>qua1>>spe1>>exp1>>ci1)
    {
        if(line!=x)
        {
            tempObj<<line<<endl<<name1<<endl<<age1<<endl<<qua1<<endl<<spe1<<endl<<exp1<<endl<<ci1<<endl;
        }
    }
    ...
    newObj<<ID<<endl<<name<<endl<<age<<endl<<qua<<endl<<spe<<endl<<exp<<endl<<ci<<endl;
    newObj.close();
...
//    cout << "Error opening file";
    ...
    while(file >> ID >> name >>age >>qua >> spe >>exp >> ci)   //!fileObj.eof()
    {
...
{
public:
    string p_name,occupation,p_city,dis,addDate,relDate;
    int p_age,p_ID;
...
void addPat()
{
    ...
    patObj<<p.p_ID<<endl<<p.p_name<<endl<<p.p_age<<endl<<p.phnumber<<endl<<p.occupation<<endl<<p.p_city<<endl<<p.dis<<endl<<p.addDate<<endl<<p.relDate<<endl<<p.bill<<endl<<dept<<endl;
    patObj.close();
...
void editPat(patient &pat)
{
    ...
    patObj.seekg(0);
    //ID,firstname,lastname,age,phnumber,occupation,city,disease,Adate,Rdate,bill,dept
    string p_name,occupation,p_city,dis,addDate,relDate,dept;
    int p_age,p_ID,count=0;
    long double phnumber,bill;
    while(patObj>>p_ID>>p_name>>p_age>>phnumber>>occupation>>p_city>>dis>>addDate>>relDate>>bill>>dept)
    {
        if(p_ID==x)
        {
            cout<<"1.ID:"<<p_ID<<endl<<"2.Name:"<<p_name<<endl<<"3.Age"<<p_age<<endl<<"4.Phone No.:"<<phnumber<<endl<<"5.Occupation:"<<occupation<<endl<<"6.City:"<<p_city<<endl<<"7.Disease/Symptoms:"<<dis<<endl<<"8.Admission date(DD/MM/YYYY):"<<addDate<<endl<<"9.Release date(DD/MM/YYYY):"<<relDate<<endl<<"10.Total Bill generated:"<<bill<<endl<<"11.Kind of Specialization required:"<<dept<<endl;
            count++;
            break;
        }
    }
    ...
    tempObj.seekp(0);
    int skip = 0;
    string name1,occupation1,p_city1,dis1,addDate1,relDate1,dept1;
    int p_age1;
...
//	else
//    	cout << "Error opening file";
    while(file>>line>>name1>>p_age1>>phnumber1>>occupation1>>p_city1>>dis1>>addDate1>>relDate1>>bill1>>dept1)
    {
        if(line!=x)
        {
            tempObj<<line<<endl<<name1<<endl<<p_age1<<endl<<phnumber1<<endl<<occupation1<<endl<<p_city1<<endl<<dis1<<endl<<addDate1<<endl<<relDate1<<endl<<bill1<<endl<<dept1<<endl;
        }
    }
    ...
    newObj.seekp (0, ios::end);
    newObj<<p_ID<<endl<<p_name<<endl<<p_age<<endl<<phnumber<<endl<<occupation<<endl<<p_city<<endl<<dis<<endl<<addDate<<endl<<relDate<<endl<<bill<<endl<<dept<<endl;
    newObj.close();
...
void dispPatDatabase()
{
    ...
    int p_age,p_ID,count=0;
    long double phnumber,bill;
    while(Obj>>p_ID>>p_name>>p_age>>phnumber>>occupation>>p_city>>dis>>addDate>>relDate>>bill>>dept)
    {
        count++;
...
void getStatus()
{
    ...
    int p_age,p_ID,count=0;
    long double phnumber,bill;
    while(file>>p_ID>>p_name>>p_age>>phnumber>>occupation>>p_city>>dis>>addDate>>relDate>>bill>>dept)
    {
        count++;
    }
...
int main()
{
    ...
    system("color 72");
    cout<<endl;
    cout<<right<<setw(60)<<setfill(' ')<<"WELCOME TO UEM HOSPITAL MANAGEMENT SYSTEM "<<endl;
    cout<<endl<<endl;
